...
ggsave(plot = full_plot, 
       filename = here("output/space/word_frequency_general.pdf"), 
       width = 12, height = 12)
...
words_of_interest <- c(
  ...
  "convergence", "cohesion", "innovation")

# Also use the following word stems 
word_stems_of_interest <- c("sustainab", "competiti", "digital")
...
word_freqs_2_line <- ggplot(
  ...
  labs(title = "Frequency of selected words in ASGS 2011-2021", 
       y="Relative frequency", 
       caption = paste("Relative frequency of words as share of total words",
                       "in the respective year.")) +
  scale_color_brewer(palette = "Dark2") +
  scale_y_continuous(
    labels = scales::percent_format(scale = 1, accuracy = 0.1),
    expand = expansion(add = c(0.3, 0.25))) +
...
ggsave(plot = word_freqs_2_line, 
       filename = here("output/space/word_frequency_selected_words.pdf"), 
       width = 8, height = 4.5)
